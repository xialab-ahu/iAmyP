...
# Train classifiers and perform hard voting on training data
hard_voting_clf = VotingClassifier(estimators=classifiers, voting='hard')
hard_voting_clf.fit(X_train_att, y_train)

# Predict using hard voting classifiers on training data
meta_X_train = np.array([clf.predict(X_train_att) for clf in hard_voting_clf.estimators_]).T
meta_X_test = np.array([clf.predict(X_test_att) for clf in hard_voting_clf.estimators_]).T

# Train soft voting meta-classifier
soft_voting_clf = VotingClassifier(estimators=classifiers, voting='soft')
soft_voting_clf.fit(meta_X_train, y_train)

# Predict using the soft voting meta-classifier
ensemble_prediction = soft_voting_clf.predict_proba(meta_X_test)[:, 1]
