...
X_train = attention_model(train_Q, train_K, train_V).detach().numpy()
X_test = attention_model(test_Q, test_K, test_V).detach().numpy()
...
classifiers = [
    ...
    AdaBoostClassifier(DecisionTreeClassifier(max_depth=10, min_samples_split=20, min_samples_leaf=5), algorithm="SAMME", n_estimators=100, learning_rate=0.8),
...
# Predictions
predictions = np.array([clf.predict_proba(X_test)[:, 1] for clf in classifiers])
...
# Particle Swarm Optimization using optunity
def optimize_func(**args):
    weights = np.array([args[key] for key in sorted(args.keys())])
    return log_loss_func(weights)

# Define search space
search_space = {'w'+str(i): [0, 1] for i in range(len(classifiers))}

# Perform optimization
optimal_weights, _, _ = optunity.minimize_structured(optimize_func, search_space, num_evals=100)
best_weights = np.array([optimal_weights['w'+str(i)] for i in range(len(classifiers))])

# Calculate ensemble predictions using optimized weights
ensemble_prediction = np.dot(best_weights, predictions)

# Calculate ensemble metrics
logloss = log_loss(y_test, ensemble_prediction)
accuracy = accuracy_score(y_test, ensemble_prediction > 0.8)
mcc = matthews_corrcoef(y_test, ensemble_prediction > 0.8)
precision = precision_score(y_test, ensemble_prediction > 0.8)
recall = recall_score(y_test, ensemble_prediction > 0.8)
f1 = f1_score(y_test, ensemble_prediction > 0.8)
confusion = confusion_matrix(y_test, ensemble_prediction > 0.8)
